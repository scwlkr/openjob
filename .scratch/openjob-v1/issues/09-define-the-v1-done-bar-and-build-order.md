...
5. Historical requirement, superseded for the maintained QA fixture: run the cross-client acceptance scenario with the existing Google-backed `@scwlkr` owner User plus Preview-only password User `@qa-two`; do not create dedicated Google test accounts. Across web and CLI, prove sign-in, Username claim, Group creation and rename, Invite Link join and rotation, Task create/edit/reassign/complete/reopen/delete, filters and stable ordering, promotion/demotion guards, kick, ban, unban, removal-induced Unassigned recovery, leave guards, and confirmed Group ending. A mutation in either client must appear correctly in the other. Real Google and Apple product acceptance remains separately owned by the current authentication issue contract.
...
`v0.1.0` is done only when both clients expose the complete product, the maintained two-User Preview acceptance and separate real-provider production acceptance pass, rollback has been proven, and no public legacy task path remains. Self-hosting packaging, public CLI distribution, notifications, recurrence, integrations, richer Tasks, and evidence-driven operational expansion remain beyond this release.
