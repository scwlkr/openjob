...
test("production is the immutable default CLI profile", () => {
  assert.deepEqual(CLI_PROFILE_NAMES, ["production", "preview-owner"]);
...
test("Preview owner resolves one environment-bound immutable tuple", () => {
  const profile = resolveCliProfile("preview-owner", {
    OPENJOB_PREVIEW_OWNER_EXPECTED_USER_ID: PREVIEW_USER_ID,
  });
  ...
  assert.deepEqual(profile, {
    ...
    credentialAccount:
      `firebase-refresh-token:preview-owner:${PREVIEW_CREDENTIAL_DIGEST}`,
...
test("profile selection is closed and Preview owner requires its 1Password identity binding", () => {
  assert.throws(
  ...
  assert.throws(
    () =>
      resolveCliProfile("preview-owner", {
      }),
    (error) =>
      error.code === "config_invalid" &&
      /OPENJOB_PREVIEW_OWNER_EXPECTED_USER_ID/u.test(error.message),
  );
  assert.throws(
    () =>
    ...
    (error) =>
      error.code === "config_invalid" &&
      /requires --profile preview-owner/u.test(error.message),
  );
});
...
test("runtime endpoint and test seams cannot split a non-test profile", () => {
  for (const environment of [
    ...
    {
      OPENJOB_PREVIEW_OWNER_GOOGLE_OAUTH_CLIENT_ID:
        "1015996869029-7rsl506o6gc6sg9d7l5kl6ant3q1t4cb.apps.googleusercontent.com",
    },
  ]) {
    assert.throws(
...
test("production and Preview use separate config and Keychain namespaces", () => {
  ...
  const preview = resolveCliProfile("preview-owner", {
    OPENJOB_PREVIEW_OWNER_EXPECTED_USER_ID: PREVIEW_USER_ID,
  });
  ...
  assert.equal(
    ...
    "/tmp/openjob-profile-test/openjob-preview-owner/config.json",
  );
  assert.notEqual(production.credentialAccount, preview.credentialAccount);
  const otherPreview = resolveCliProfile("preview-owner", {
    OPENJOB_PREVIEW_OWNER_EXPECTED_USER_ID: `user_${"b".repeat(32)}`,
  });
